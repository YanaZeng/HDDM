function g = pc(X,order,cond_ind_test,pars,maxFanIn,alpha,cache)
% function g = pc(X,cond_ind_test,pars,maxFanIn,alpha,cache)
...
% cache         - cache results of (conditional) independence tests? (very slow if
%                 p is large, but may be faster if n is large and p is small)
%
% OUTPUTS:
...
%Yana:'thin' the graph acc. to order
for i = 1:(n-1)
    for j = (i+1):n
        g(order(j),order(i)) = 0;
    end
...
% find graph skeleton
for s=0:maxFanIn % iteratively increase size of conditioning setu
    for i=1:n
        ...
        for j=adjSet
            ...
            % We cannot separate X1 and Z using adjSet_X1.
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
